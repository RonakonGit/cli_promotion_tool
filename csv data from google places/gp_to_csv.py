...
import pandas as pd
import time
def places_to_csv(location, keyword, radius ):    
    API_KEY = 'google places api key'

    google_places = GooglePlaces(API_KEY)

    #take input search information
    location1 = str(location)
    ...
    data.to_csv('ronak_' + keyword1 + '_in_' + location1 + '.csv', encoding='utf-8')
    return ("processd one file")


file1 = open('city-list.txt', 'r') 
Lines = file1.readlines()
for i in Lines :
    try :
        places_to_csv( i,"gym","500000")
    except :
        print("some error occured - skipping")
